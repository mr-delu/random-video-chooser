...
public class NoteWindow {
	...
	public static void run(UserSession sessione, RandomVideo rv, long timeSession) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NoteWindow window = new NoteWindow(sessione,rv,timeSession);
	...
	public NoteWindow(UserSession sessione, RandomVideo rv, long timeSession) {
		initialize(sessione,rv,timeSession);
	}

	
	private void initialize(UserSession sessione, RandomVideo rv,long timeSession) {
		frame = new JFrame();
		...
		btnTermina.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				sessione.complete(rv,timeSession);
